/* moria.c — monolithic Umoria 5.6 (Linux x86-64)
   *
   ...
   *
   * Generated from Umoria 5.6 source, dead platform code stripped.
   */

  /* Pull in all system headers first to avoid implicit-declaration conflicts */
  #include <stdio.h>
  #include <stdlib.h>
  #include <string.h>
  #include <unistd.h>
  ...
  /* Bring in local path overrides before config.h sees them */
  #include "unix/local_paths.h"
  ...
  if (highscore_fp == NULL)
    {
      ...
      highscore_fp = fopen(MORIA_TOP, "w+");
      if (highscore_fp == NULL)
        {
